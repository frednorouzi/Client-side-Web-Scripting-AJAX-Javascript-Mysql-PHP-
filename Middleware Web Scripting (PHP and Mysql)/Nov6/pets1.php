...
<head>	
	...
	.m {margin-top: 30px; margin-right: auto; margin-left: auto;}
	</style>
</head>
<body>
...
<label><input type='text' required name='fname'>Fname</label>
...
list($ownerid, $fname, $lname, $city) = $row;


print "<form method='post' action='pets1.php'>
<fieldset>
<legend>Edit Owner</legend>
<input type='hidden' name='ownerid' value='$ownerid'>
<label><input type='text' name='fname' value='$fname'>Fname</label>
<label><input type='text' name='lname' value='$lname'>Lname</label>
<label><input type='text' name='city' value='$city'>City</label>
<label><input type='submit' name='editowner2' value='Edit'></label>
</fieldset>
</form>";
...
$reName = "/^[a-zA-Z]+(([\'\- ][a-zA-Z])?[a-zA-Z]*)*$/";
$reCity = "/^[a-zA-Z ]+$/";
	if(preg_match($reName, $fname) && preg_match($reName, $lname) && 
	preg_match($reCity, $city))
	{
	$fname = fixnames($fname);
	$lname = fixnames($lname);
	$city = fixnames($city);
	$query = "select * from owners where fname='$fname' and lname='$lname'
	and ownerid != '$ownerid'";
	$result = mysqli_query($link, $query);
		if(mysqli_num_rows($result) == 0)
		{
		$queryupdate = "update owners set fname='$fname', lname='$lname',
		city='$city' where ownerid='$ownerid'";
		mysqli_query($link, $queryupdate);
		print "<h2>$fname $lname was updated</h2>";
		}
		else
		{
		print "<h2>There is someone else with that name</h2>";
		}
	
	}
	else
	{
	print "<h2>Fill out form</h2>";
	}
...
if(isset($_GET['animalownership']))
{
$ownerid = $_GET['ownerid'];
$fname = $_GET['fname'];
$lname = $_GET['lname'];
$query = "select pets.petid, petname, species from pets, ref
where ref.petid=pets.petid and ref.ownerid='$ownerid'";
$result = mysqli_query($link, $query);

print "<table border='1'>
<caption>Pets owned by $fname $lname</caption>
<tr><th>Delete</th><th>Petname</th><th>Species</th></tr>";
	while($row = mysqli_fetch_row($result))
	{
	list($petid, $petname, $species) = $row;
	print "<tr>
		<td><a href='pets1.php?delpet=yes&petid=$petid'>Delete Pet</a></td>
